...
#define KEY_UP(VK_NONAME) ((GetAsyncKeyState(VK_NONAME) & 0x8000) ? 0 : 1)//判断这个键是否松口 
...
#define GetWindow() GetForegroundWindow();//获取窗口信息 
/*
鼠标左键 : MOUSE_MOVED 
鼠标右键 ：MOUSE_EVENT
鼠标滚轮 : MOUSE_WHEELED
...
void Window_Hide(HWND hWnd){ShowWindow(hWnd,0);}
void Window_Show(HWND hWnd){ShowWindow(hWnd,1);}
...
void click_left(){//点击鼠标右键
    mouse_event(MOUSEEVENTF_LEFTDOWN,0,0,0,0); //按下鼠标左键 
    Sleep(5);//要留给某些应用的反应时间   
    mouse_event(MOUSEEVENTF_LEFTUP,0,0,0,0); //松口鼠标左键 
}
void click_right(){//点击鼠标右键 
    mouse_event(MOUSEEVENTF_RIGHTDOWN,0,0,0,0);  
    Sleep(5);   
    mouse_event(MOUSEEVENTF_RIGHTUP,0,0,0,0);  
}
void GOTO(int x,int y){//移动光标到窗口中的位置 
